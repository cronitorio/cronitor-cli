...
Local login listens only on `127.0.0.1:8319` and receives the callback automatically. If that port is occupied, close the other login attempt or use `--no-browser`. Login expires after five minutes; use `--timeout 10m` if needed. These instructions require version 33.7 or later: version 33.6 used a device flow that is incompatible with our authentication provider's External Sign-in. Existing API keys continue working; upgrade to 33.7 or later for browser login.
